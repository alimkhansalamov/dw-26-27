...
De tijden omvatten theorie, oefeningen en het bekijken van de oplossingen.

| Onderdeel                                  |            Tijd |
| ------------------------------------------ | --------------: |
| 1. Startcheck                              |      10 minuten |
| 2. HTML en CSS: de noodzakelijke basis     |      15 minuten |
| 3. Variabelen, voorwaarden en functies     |      30 minuten |
| 4. Arrays en objecten                      |      35 minuten |
| Pauze                                      |      15 minuten |
| 5. Moderne JavaScript en immutable updates |      30 minuten |
| 6. Events en modules                       |      25 minuten |
| 7. Asynchrone JavaScript en API’s          |      35 minuten |
| 8. Geïntegreerde oefening                  |      35 minuten |
| 9. Eindcheck                               |      10 minuten |
| **Totaal**                                 | **240 minuten** |
...
Bekijk de oplossingen nadat je zelf een poging hebt gedaan.
...
```js
console.log(price + quantity);         // "202"
console.log(Number(price) + quantity); // 22
```

Bij de eerste expressie zorgt de string ervoor dat `+` tekst samenvoegt.
...
`==` voert typeconversie uit. `===` doet dat niet.
...
De functie heeft accolades, maar geen `return`.
...
`map` maakt hier een nieuwe array zonder de oorspronkelijke getallen te wijzigen.
...
```js
const calculateOrderTotal = (price, quantity, isMember) => {
...
Voorraadlabel, inclusief de extra controle:

```js
const getStockLabel = (stock) => {
  if (stock < 0) {
    return "Ongeldige voorraad";
  }
...
Na elke `return` stopt de functie. Bij `stock <= 5` weten we daardoor al dat de voorraad niet negatief of nul is.
...
const missingProduct = products.find((product) => {
  return product.id === 99;
});

if (missingProduct === undefined) {
  console.log("Product niet gevonden");
} else {
  console.log(missingProduct.name);
}
```
...
console.log(searchProducts(products, "  MUIS  "));
```

Bij een lege zoektekst geeft `includes("")` voor elke productnaam `true`. Daardoor krijg je alle producten terug.

## Oefening 4 — Immutable updates
...
const cartWithoutKeyboard = cart.filter((product) => {
  return product.id !== 1;
});

const { name } = customer;

const city = customer.address?.city ?? "Nog niet opgegeven";
...
`quantity` is hier een verkorte schrijfwijze voor `quantity: quantity`.

## Oefening 5 — Events en modules
...
```js
export const createProductMessage = (name) => {
...
```js
import { createProductMessage } from "./helpers.js";
...
form.addEventListener("submit", (event) => {
  ...
  if (name === "") {
    message.textContent = "Vul een productnaam in.";
    return;
  }

  message.textContent = createProductMessage(name);
...
De browser houdt een volledig leeg verplicht veld al tegen. De controle met `trim()` vangt daarnaast invoer op die uitsluitend uit spaties bestaat.
...
const loadUsers = async () => {
  ...
  try {
    ...
    status.textContent =
      "Gebruikers laden is mislukt. Probeer opnieuw.";
...
Gebruik de HTML en CSS uit de opdracht.
...
const renderUsers = (visibleUsers) => {
  const cards = visibleUsers.map(createUserCard);
  ...
  status.textContent =
    visibleUsers.length === 0
      ? "Geen gebruikers gevonden."
      : `${visibleUsers.length} van ${users.length} gebruikers getoond.`;
};
...
| ---------------- | -------------------------------------------- |
| `fetchUsers`     | Gegevens ophalen en HTTP-fouten signaleren   |
| `createUserCard` | Eén gebruiker omzetten naar een HTML-element |
| `renderUsers`    | Een gegeven lijst op de pagina tonen         |
| `applyFilter`    | De zichtbare gebruikers bepalen              |
| `loadUsers`      | Laden, data en feedback coördineren          |

Extra: zoeken op naam of e-mailadres.

Vervang de filtercallback door:
...
1. Gebruik `let` wanneer je de variabele later een nieuwe waarde moet toekennen.
2. De functie mist `return`. Correct: `const add = (a, b) => a + b;`.
3. Gebruik `filter` voor alle passende producten en `find` voor het eerste passende product.
4. `find` geeft `undefined` terug.
5. Beide variabelen verwijzen naar hetzelfde object.
6. `const updatedProduct = { ...product, price: 50 };`.
7. `||` gebruikt de fallback voor alle falsy waarden. `??` alleen voor `null` en `undefined`.
8. `handleClick` geeft de functie door. `handleClick()` voert ze onmiddellijk uit.
9. Een HTTP-fout zoals `404` of `500` wijst de Promise van `fetch` niet automatisch af.
10. Laden, resultaten, een leeg resultaat en een fout.
